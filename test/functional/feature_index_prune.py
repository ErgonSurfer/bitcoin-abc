...
class FeatureIndexPruneTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 4
        self.extra_args = [
            ["-fastprune", "-prune=1", "-blockfilterindex=1", "-noparkdeepreorg"],
            ["-fastprune", "-prune=1", "-coinstatsindex=1", "-noparkdeepreorg"],
            [
                "-fastprune",
                "-prune=1",
                "-blockfilterindex=1",
    ...
    def sync_index(self, height):
        ...
        self.wait_until(
            lambda: self.nodes[1].getindexinfo() == expected_stats, timeout=150
        )
    ...
    def run_test(self):
        ...
        for node in stats_nodes:
            assert node.gettxoutsetinfo(hash_type="muhash", hash_or_height=height_hash)[
                "muhash"
            ]
