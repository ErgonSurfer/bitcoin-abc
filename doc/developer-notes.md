...
Ergon ABC is a multi-threaded application, and deadlocks or other
...
Ergon ABC can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Ergon ABC, where everyone uses
...
pay attention to for reviewers of Ergon ABC code.
...
submitted to Ergon ABC so that they can be integrated quickly.
...
- src/leveldb
  ...
  - **Note**: Follow the instructions in [Upgrading LevelDB](#upgrading-leveldb)
    when merging upstream changes to Ergon ABC.
...
- src/secp256k1
  - Upstream at <https://github.com/bitcoin-core/secp256k1/> ; actively maintained
    ...
    Ergon ABC is using a modified version of libsecp256k1, some changes might
    be directly submitted to Ergon ABC.
...
would be to revert the upstream fix before applying the updates to Ergon ABC's
