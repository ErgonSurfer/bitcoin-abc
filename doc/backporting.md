...
The official Ergon ABC guide to backporting code from Bitcoin Core. When searching
for items to backport, especially be on the lookout for bug fixes, code cleanup, and
important refactors, as these help improve Ergon ABC despite consensus-level differences
...
3. Tag the fork commit as `fork-commit`. Ergon ABC was forked from Bitcoin Core
...
Before you begin backporting commits, you will need to add an additional remote to your Ergon ABC repo.
...
The Ergon ABC monorepo includes subprojects that are stored in subdirectories of the main repository.
