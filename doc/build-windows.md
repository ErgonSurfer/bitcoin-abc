...
Below are some notes on how to build Ergon ABC for Windows.

The options known to work for building Ergon ABC on Windows are:

* On Linux, using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Debian Bookworm is recommended
and is the platform used to build the Ergon ABC Windows release binaries.
...
The cmake version packaged with Ubuntu Bionic is too old for building Building Ergon ABC.
...
Note that for WSL the Ergon ABC source path MUST be somewhere in the default mount file system, for
...
It's not possible to build the Ergon ABC code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Ergon ABC source code).
