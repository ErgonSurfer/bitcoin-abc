# Ergon ABC 0.28.8 Release Notes

Ergon ABC version 0.28.8 is now available from:
...
Ergon ABC will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`bitcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Ergon ABC will now just log warnings instead of creating new wallets with
